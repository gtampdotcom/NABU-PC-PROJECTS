; *****************************************************************************
; * NABU PC VGM Player v0.4 by GTAMP (c) 2026
; * MIT licensed
; * v0.4 code was 95% Gemini with some help from Claude Sonnet 4.5 free
; * v0.3 code was 100% by the free version of Gemini 3 using the fast setting
...
.is_vt: inc b               
.save_term:
    ld a, b : ld (term_mode), a
    or a : jr z, .adm_clr
    ; VT100 Reset
    ld e, 27 : ld c, F_CONOUT : call BDOS
    ld e, 'c' : ld c, F_CONOUT : call BDOS
    jr .init_sys
.adm_clr: ld e, 26 : ld c, F_CONOUT : call BDOS
...
.init_sys:
    ...
    ld a, 36 : call set_ansi_color ; Cyan
    ld de, msg_header : ld c, F_PRTSTR : call BDOS
    ld a, 0 : call set_ansi_color 
...
; *****************************************************************************
; * TERMINAL & GD3 HELPERS
; *****************************************************************************

set_ansi_color:
    push af
    ld a, (term_mode) : or a : jr z, .pop_ret 
    ld e, 27 : ld c, F_CONOUT : call BDOS    
    ld e, '[' : ld c, F_CONOUT : call BDOS   
    pop af : push af
    or a : jr nz, .not_reset
    ld e, '0' : ld c, F_CONOUT : call BDOS   
    jr .end
.not_reset:
    ld e, '3' : ld c, F_CONOUT : call BDOS   
    pop af : push af
    sub 30 : add a, '0' : ld e, a : ld c, F_CONOUT : call BDOS 
.end:
    ld e, 'm' : ld c, F_CONOUT : call BDOS   
.pop_ret:
    pop af : ret

; --- Refactored GD3 String Printer (Using IY for Data) ---
gd3_print_string:
.lp:
    ld a, (iy+0) : inc iy
    ld b, (iy+0) : inc iy
    ...
    ld a, b : or a : jr nz, .lp  ; Filter non-ASCII
    ld a, c
    cp 32 : jr c, .lp            
    cp 127 : jr nc, .lp          
    
    ld e, a : ld c, F_CONOUT : call BDOS
    jr .lp

.done_tag:
    ld de, msg_crlf : ld c, F_PRTSTR : call BDOS
.skip_alt:
    ld a, (iy+0) : inc iy : ld b, (iy+0) : inc iy
    or b : jr nz, .skip_alt
    ret
...
do_ay:
    ld hl, (vgm_ptr) : ld c, (hl) : inc hl : ld b, (hl) : inc hl : ld (vgm_ptr), hl
    push hl : ld hl, ay_shadow : ld e, c : ld d, 0 : add hl, de : ld (hl), b : pop hl
    ld a, c : cp 7 : jr nz, .o 
    ld a, b : and %00111111 : or %01000000 : ld b, a
.o: ld a, c : out (AY_REG_SEL), a : ld a, b : out (AY_DATA_WR), a : jp vgm_main                
...
.innr: dec de : ld a, d : or e : jr nz, .innr : pop bc : djnz .outr : jp vgm_main                

do_wait_n: ld hl, (vgm_ptr) : inc hl : inc hl : ld (vgm_ptr), hl : jp do_wait

refill_needed:
    call silence_ay
    ld a, 22 : call set_cursor 
    ld a, 33 : call set_ansi_color 
    ld de, msg_refill : ld c, F_PRTSTR : call BDOS
    ld hl, (cur_part) : inc hl : ld (cur_part), hl : call update_ui_parts
    call refill_buffer
    ld a, 22 : call set_cursor : ld de, msg_blank : ld c, F_PRTSTR : call BDOS 
    call restore_ay : jp vgm_main

auto_advance:
    ld a, (loop_flag) : or a : jr nz, do_loop : jp skip_next
...
do_loop:
    ...
    xor a : ld (first_load), a : call refill_buffer
    call update_ui_parts : call update_ui_time : call restore_ay : jp vgm_main

; *****************************************************************************
; * GD3 AND UI
; *****************************************************************************
read_gd3_tags:
    ld hl, FCB : ld de, fcb_backup : ld bc, 36 : ldir
    ld hl, FCB+1 : ld de, GD3_FCB+1 : ld bc, 11 : ldir
    xor a : ld (GD3_FCB), a : ld (GD3_FCB+12), a : ld (GD3_FCB+32), a
    ld de, GD3_FCB : ld c, F_OPEN : call BDOS
    inc a : jp z, .no_gd3
    ld de, TBUFF : ld c, F_SETDMA : call BDOS
    ld de, GD3_FCB : ld c, F_READ : call BDOS
    or a : jp nz, .no_gd3
    ld hl, (TBUFF + $14) : ld de, (TBUFF + $16)  
    ld a, h : or l : or d : or e : jp z, .no_gd3
    push de : ld de, $0014 : add hl, de : pop de : jr nc, .no_carry : inc de  
.no_carry:
    ld b, 7
.div_loop: srl d : rr e : rr h : rr l : djnz .div_loop
    ld (GD3_FCB+33), hl : xor a : ld (GD3_FCB+35), a
    ld b, 8 : ld de, buffer_zone
.load:
    push bc : push de : ld c, F_SETDMA : call BDOS
    ld de, GD3_FCB : ld c, F_READ_RAND : call BDOS
    pop de : pop bc : or a : jr nz, .load_done 
    ld hl, (GD3_FCB+33) : inc hl : ld (GD3_FCB+33), hl
    ld hl, 128 : add hl, de : ex de, hl : djnz .load
.load_done:
    ld hl, buffer_zone : ld bc, 1024
.find:
    ld a, (hl) : cp 'G' : jr nz, .next : inc hl : ld a, (hl) : cp 'd' : jr nz, .back : inc hl : ld a, (hl) : cp '3' : jr nz, .back : jr .found
.back: dec hl
.next: inc hl : dec bc : ld a, b : or c : jr nz, .find : jr .no_gd3
.found:
    ld de, 10 : add hl, de
    ld a, (hl) : or a : jr z, .no_gd3  
    push hl : pop iy           ; IY = Start of GD3 Strings
    ld ix, GD3_LABEL_TABLE
.print_loop:
    ld l, (ix+0) : ld h, (ix+1) : ld a, l : or h : jr z, .done
    push hl : ld a, 36 : call set_ansi_color : pop de : ld c, F_PRTSTR : call BDOS 
    ld a, 0 : call set_ansi_color  
    
    ; Check if current GD3 string is empty (0000)
    ld a, (iy+0) : ld b, (iy+1) : or b 
    jr z, .skip_empty
    call gd3_print_string 
    jr .next_tag
.skip_empty:
    inc iy : inc iy : inc iy : inc iy ; Skip Eng(0) and Alt(0)
    ld de, msg_crlf : ld c, F_PRTSTR : call BDOS
.next_tag:
    inc ix : inc ix : jr .print_loop
.done: ld de, msg_crlf : ld c, F_PRTSTR : call BDOS
.no_gd3:
    ld de, TBUFF : ld c, F_SETDMA : call BDOS
    ld hl, fcb_backup : ld de, FCB : ld bc, 36 : ldir : ret
...
.adm3a:
    ...
    ld e, 32 : ld c, F_CONOUT : call BDOS : pop af : ret
.vt100:
    ld e, 27 : ld c, F_CONOUT : call BDOS : ld e, '[' : ld c, F_CONOUT : call BDOS
    pop af : push af 
    add a, 3 : ld l, a : ld h, 0 : call prt_num_vt
    ld e, ';' : ld c, F_CONOUT : call BDOS : ld e, '1' : ld c, F_CONOUT : call BDOS : ld e, 'H' : ld c, F_CONOUT : call BDOS
    pop af : ret

prt_num_vt:
    ld a, l : cp 10 : jr c, .single
    ld b, '0'
.lp: sub 10 : inc b : cp 10 : jr nc, .lp
    ld c, a : ld e, b : push bc : ld c, F_CONOUT : call BDOS : pop bc : ld a, c
.single: add a, '0' : ld e, a : ld c, F_CONOUT : call BDOS : ret

update_ui_file: 
    ld a, 10 : call set_cursor 
    ld a, 32 : call set_ansi_color ; Green
    ld de, msg_playing : ld c, F_PRTSTR : call BDOS 
    ld a, 0 : call set_ansi_color 
    call print_filename : ret
update_ui_speed: 
    ld a, 11 : call set_cursor 
    ld a, 32 : call set_ansi_color ; Green
    ld de, msg_speed_lbl : ld c, F_PRTSTR : call BDOS
    ld a, 0 : call set_ansi_color 
    ld a, (speed_val) : ld l, a : ld h, 0 : call prt_num_vt : ld de, msg_blank_sm : ld c, F_PRTSTR : call BDOS : ret
update_ui_loop: 
    ld a, 12 : call set_cursor 
    ld a, 32 : call set_ansi_color ; Green
    ld de, msg_loop_lbl : ld c, F_PRTSTR : call BDOS
    ld a, (loop_flag) : or a : ld de, msg_lon : jr nz, .p : ld de, msg_loff
.p: ld c, F_PRTSTR : call BDOS : ret
update_ui_parts: 
    ld a, 13 : call set_cursor 
    ld a, 32 : call set_ansi_color ; Green
    ld de, msg_part_lbl : ld c, F_PRTSTR : call BDOS
    ld a, 0 : call set_ansi_color 
    ld hl, (cur_part) : call prt_num_vt_16 : ld de, msg_of : ld c, F_PRTSTR : call BDOS
    ld hl, (total_parts) : call prt_num_vt_16 : ret
update_ui_time: 
    ld a, 14 : call set_cursor 
    ld a, 32 : call set_ansi_color ; Green
    ld de, msg_time_lbl : ld c, F_PRTSTR : call BDOS
    ld a, 0 : call set_ansi_color 
    ld hl, (time_min) : call prt_num_vt_16 : ld e, ':' : ld c, F_CONOUT : call BDOS
    ld a, (time_sec) : cp 10 : jr nc, .s : ld e, '0' : ld c, F_CONOUT : call BDOS
.s: ld l, a : ld h, 0 : call prt_num_vt : ret
draw_ui_legend:
    ld a, 16 : call set_cursor 
    ld a, 33 : call set_ansi_color ; Yellow
    ld de, msg_legend : ld c, F_PRTSTR : call BDOS 
    ld a, 0 : call set_ansi_color 
    ret
pause_now:
    call silence_ay : ld a, 22 : call set_cursor 
    ld a, 33 : call set_ansi_color ; Yellow
    ld de, msg_pause : ld c, F_PRTSTR : call BDOS
.w: ld e, $FF : ld c, F_DIRIO : call BDOS : or a : jr z, .w
    ld a, 22 : call set_cursor : ld de, msg_blank : ld c, F_PRTSTR : call BDOS : call restore_ay : ret
...
play_current:
    ...
    ld a, (term_mode) : or a : jr z, .adm_clr
    ld e, 27 : ld c, F_CONOUT : call BDOS : ld e, 'c' : ld c, F_CONOUT : call BDOS
    jr .tag_read
.adm_clr: ld e, 26 : ld c, F_CONOUT : call BDOS
.tag_read:
    ld a, 36 : call set_ansi_color
    ld de, msg_header : ld c, F_PRTSTR : call BDOS
    ld a, 0 : call set_ansi_color
    ld de, msg_crlf : ld c, F_PRTSTR : call BDOS
    ld de, msg_crlf : ld c, F_PRTSTR : call BDOS
    call read_gd3_tags : call draw_ui_legend
    ...
    xor a : ld (first_load), a : ld (time_sec), a : ld hl, 0 : ld (time_min), hl : ld (wait_count), hl
    call refill_buffer : jp vgm_main
...
silence_ay:
    ld a, 8 : call .zero : ld a, 9 : call .zero : ld a, 10 : call .zero : ld a, 13 : call .zero : ret
.zero: out (AY_REG_SEL), a : xor a : out (AY_DATA_WR), a : ret
...
.offs: ld hl, buffer_zone + $34 : add hl, de : ld (vgm_ptr), hl : ld a, 1 : ld (first_load), a : ret
...
exit_all:
    call silence_ay : ld a, (term_mode) : or a : jr z, .adm
    ld a, 0 : call set_ansi_color 
.adm: ld a, 23 : call set_cursor : ld de, msg_crlf : ld c, F_PRTSTR : call BDOS
    ld a, (orig_user) : ld e, a : ld c, F_USER : call BDOS
...
FCB_TEMP:          ds 36
msg_header:        db "NABU PC VGM Player v0.4 by GTAMP", "$"
msg_playing:       db "File:     $", "$"
...
GD3_LABEL_TABLE:
    dw gd3_lbl_title, gd3_lbl_game, gd3_lbl_sys
    dw gd3_lbl_comp, gd3_lbl_date, gd3_lbl_vgm, gd3_lbl_notes, 0
gd3_lbl_title:     db "Track:    $", "$" ; Explicit double terminator for safety
gd3_lbl_game:      db "Game:     $", "$"
gd3_lbl_sys:       db "System:   $", "$"
gd3_lbl_comp:      db "Composer: $", "$"
gd3_lbl_date:      db "Release:  $", "$"
gd3_lbl_vgm:       db "VGM by:   $", "$"
gd3_lbl_notes:     db "Notes:    $", "$"
