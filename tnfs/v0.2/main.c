/* NABU CP/M TNFS client v0.2 by GTAMP (c) 2026
...
void load_dir() {
    ...
    move_cursor(0, 0); printf("TNFS v0.2 | Scanning...%-16s", "");
    ...
    while (g_fileCount < 150) {
        ...
        /* Probe with OPENDIR to determine if entry is a directory.
           ".." is always a dir; for others we attempt OPENDIR and
           check the status byte — 0x00 means success (it is a dir). */
        if (strcmp(entryName, "..") == 0) {
            g_fileList[g_fileCount].isDir = 1;
        } else {
            char probePath[128];
            strcpy(probePath, g_currentPath);
            if (probePath[strlen(probePath)-1] != '/') strcat(probePath, "/");
            strcat(probePath, entryName);
            strcpy((char*)&g_txBuf[4], probePath);
            if (tn_xchg(CMD_OPENDIR, (uint16_t)strlen(probePath) + 1) && g_buf[4] == 0x00) {
                /* it opened as a directory — close the handle */
                g_txBuf[4] = g_buf[5]; tn_xchg(CMD_CLOSEDIR, 1);
                g_fileList[g_fileCount].isDir = 1;
            } else {
                g_fileList[g_fileCount].isDir = 0;
            }
        }
        g_fileCount++;
    }
...
void full_redraw(uint8_t cursor, uint8_t offset) {
    ...
    printf("TNFS v0.2 | Path: %-20s", g_currentPath);
...
/* Save the index of the last used server to TNFS.INI */
...
void draw_server_menu(uint8_t cursor, uint8_t offset) {
    ...
    move_cursor(0, 0);
    printf("TNFS NABU v0.2 by GTAMP | %s", g_isVT100 ? "VT100" : "ADM-3A");
...
uint8_t select_server(uint8_t initialCursor) {
    ...
    /* Position the view to show the initial cursor */
    if (cursor >= SERVER_PAGE) offset = cursor - SERVER_PAGE + 1;
