...
/* ---------- tuneable limits ---------- */
#define PAGE_SIZE    20
#define MAX_SERVERS  20   /* screen can't show more than 20 anyway */
#define MAX_FILES    20   /* one screen-page of entries in RAM at a time */
...
uint8_t get_key(void) {
    if (g_rawKeys) {
        ...
        if (raw == 0x94) return 0;           /* modifier-only, ignore */
...
static void write_submit(const char *cmd) {
    FILE *fp;
    uint8_t sub_buf[128];
    int i;
    int cmd_len = strlen(cmd);
    if (cmd_len > 126) cmd_len = 126;
    ...
    if (fp) {
        ...
        sub_buf[0] = (uint8_t)cmd_len;

        // 3. Copy the command as uppercase (CCP requirement)
        for (i = 0; i < cmd_len; i++) {
            sub_buf[i + 1] = (uint8_t)toupper(cmd[i]);
        }
...
void load_dir(void) {
    move_cursor(0, 0); printf("TNFS v0.3 | Loading... %-16s", "");
    g_dirPage = 0;
...
void full_redraw(uint8_t cursor) {
    ...
    printf("TNFS v0.3 | %-26s", g_currentPath);
...
void draw_server_menu(uint8_t cursor, uint8_t offset) {
    ...
    move_cursor(0, 0);
    printf("TNFS NABU v0.3 by GTAMP");
    ...
    move_cursor(23, 0); printf("W/S:Move A/D:Page GO:Connect Q:Quit");
}
