...
/* -----------------------------------------------------------------------
   ...
   ----------------------------------------------------------------------- */
#define STR_VERSION       "TNFS v0.6"
#define STR_TITLE         "NABU TNFS v0.6 by GTAMP"
...
static void write_submit(void) {
    ...
    uint8_t cmd_len = sizeof(STR_TEMP_SUB) - 1;
...
static uint8_t pload_exec(const char *name8, const char *ext3) {
    ...
    if (result == 0xFF) { printf("\nFILE NOT ON CLOUD"); return 0; }

    result = (uint8_t)bdos(P_LOAD, (unsigned int)fcb);
    if (result != 0) { printf("\nP_LOAD ERROR %02X", result); return 0; }
